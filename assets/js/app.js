...
const getCurrentWeatherData = async city => {
  ...
  try {
    ...
    if (typeof data == 'undefined') {
      ...
      ).innerHTML += `<li class="nav-search-result-li btnn" onclick= "setWeatherData1()">
...
const setWeatherData = async (citY) => {
  ...
  try {
    ...
    document.querySelector('.day1 img').src = iconUrl1
    document.querySelector('.day2 img').src = iconUrl2
    document.querySelector('.day3 img').src = iconUrl3
    document.querySelector('.day4 img').src = iconUrl4
    ...
    document.querySelector(
      '.rotate-icon'
    ).style.transform = `rotate(${data[0].wind_dir})`
    ...
    document.querySelector('.visibility-deg').innerHTML =
      String(data[0].vis).split('.')[0] +
      '.' +
      String(data[0].vis).split('.')[1][1];
